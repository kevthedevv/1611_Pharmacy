...
namespace Pharmacy
{
    public partial class Sales : Form
    {
        ...
        private void button_ViewSalesReport_Click(object sender, EventArgs e)
        {
            ...
            cmd = new SqlCommand("select transactionid, total, discount, subtotal, vatable, totaldue, cashtender, change, dateandtime from postotalsalepertransaction where convert (datetime,dateandtime) >='"+ dtp_SalesFrom.Value.ToShortDateString() + "' and convert (datetime,dateandtime) <= '" + dtp_SalesTo.Value.ToString() + "';", conn);
